/*
 ...
 - 不修改 system_role_menu；角色授权需通过角色管理或独立 SQL 处理
 ...
 说明：
 - 超级管理员会通过现有权限逻辑自动看到全部菜单与按钮；普通角色后续在角色管理中按需授权。
 - 如果页面只显示“详情 / 预览 / 接入映射”等查询类操作，通常是当前账号缺少 create/update/delete 权限，不是前端按钮代码丢失。
 - 直接执行本文件修改菜单后，请重新登录后台，刷新前端权限缓存与动态路由缓存。
