...
4. 通过 CDC 或主动推送发往 Kafka `config-topic`
5. Flink 订阅配置流
...
```latex
Kafka event topic   --->  eventStream  ---\
                                           connect ---> KeyedBroadcastProcessFunction ---> decisionStream
Kafka config topic  --->  configStream  ---/
```
...
#### 方案 B：多分区，但按 `sceneCode` 做 key，并用 `versionNo` 做防乱序判断
...
Kafka、CDC、下游重试都可能导致同一个版本消息重复到达。Flink 端要能正确处理：
